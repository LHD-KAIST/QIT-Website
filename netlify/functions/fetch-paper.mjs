/**
 * /.netlify/functions/fetch-paper?id=<doi | arXiv id | url>
 * Returns normalized paper metadata from arXiv (preprints) or Crossref (DOIs),
 * so the /admin/paper tool can pre-fill a new publication entry.
 */
function detect(raw) {
  const s = (raw || '').trim();
  let m =
